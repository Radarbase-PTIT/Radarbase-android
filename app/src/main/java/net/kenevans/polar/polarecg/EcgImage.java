...
class EcgImage {
    ...
    static Bitmap createImage(Bitmap logo,
                              String patientName,
                              String date,
                              String id,
                              String firmware,
        ...
        float valueStep = 200.f / ((float) IQRSConstants.FS * 8);
        for (double val : ecgvals) {
            ...
            if (index == 0) {
                ...
                index++;
                continue;
            } else if (index == 8 * IQRSConstants.FS) {
                offsetX -= (8 * IQRSConstants.FS) * valueStep;
                offsetY += 60;
            } else if (index == 16 * IQRSConstants.FS) {
                offsetX -= (8 * IQRSConstants.FS) * valueStep;
                offsetY += 60;
            } else if (index == 24 * IQRSConstants.FS) {
                offsetX -= (8 * IQRSConstants.FS) * valueStep;
                offsetY += 60;
            } else if (index > 32 * IQRSConstants.FS) {
